        ...
        if call.data == "Давай пошутим":
            ...
            def text_messages(message):
                if message.text == 'Привет':
                    ...
                    bot.send_message(call.message.chat.id, "Доброе день")
                elif message.text == 'Сколько тебе лет?':
                    bot.send_message(call.message.chat.id, "Вопрос некорректный и скорее бесполезный. Я робот(программа) Меня запустили на сервере 6 Сентября 2020 года. n\Я не старею, а только совершенствуюсь. И работаю пока люди во мне n\нуждаются")
                elif message.text == 'Прости, если обидел':
                    bot.send_message(call.message.chat.id, "Да ну, что вы. Я не обижаюсь. n\Роботы не обижаются и программы тоже)). n\Расскажи что-нибудь веселенькое-:)")
